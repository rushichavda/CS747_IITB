...
class MDP:
    def __init__(self, file, patience=5, eps=1e-8) -> None:
        ...
        self.eps = eps
        i = 3
        self.transition_matrix = np.zeros(shape=[self.num_states, self.num_states, self.num_actions], dtype=np.float__)
        self.reward_matrix = np.zeros(shape=[self.num_states, self.num_states, self.num_actions], dtype=np.float__)
    ...
    def plan(self, algo, print_output=True):
        if algo == "vi":
            ...
            prev_V = np.zeros(shape=[self.num_states, 1], dtype=np.float__)
            curr_V = np.zeros(shape=[self.num_states, 1], dtype=np.float__)
            curr_patience = 0
            ## Value iteration 
